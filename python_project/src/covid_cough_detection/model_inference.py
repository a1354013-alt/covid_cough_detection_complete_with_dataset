    ...
    def load_model(self, model_path: str) -> None:
        try:
            ...
            if state_dict is not None:
                ...
                # For directly saved modules, verify it has expected attributes
                required_attrs = ["conv1", "conv2", "conv3", "fc1", "fc2"]
                for attr in required_attrs:
                    if not hasattr(loaded, attr):
                        raise ValueError(
                            f"Model architecture mismatch. Missing attribute '{attr}'. "
                            f"Expected SimpleConvNet architecture."
                        )
                self.model = loaded
            else:
